	...
	Properties
	{
		...
		SubShader
		{
			...
			Pass
			{
				...
				float4 _DiffuseScatteringLookup_ST;
				...
				{
					float4 uv = float4(0.5 * nDotL + 0.5, subsurfaceScattering, 0, 0);
				...
				fixed4 frag(Input IN) : SV_Target
				{
					...
					half3 diffuseNormal;
					diffuseNormal.x = dot(IN.tspace0, tDiffuseNormal);
					diffuseNormal.y = dot(IN.tspace1, tDiffuseNormal);
					diffuseNormal.z = dot(IN.tspace2, tDiffuseNormal);
					...
					float3 normalB = normalize(lerp(diffuseNormal, normal, 0.3));
					...
					float3 diffuseTerm = DisneyDiffuse(nv, nl, lh, roughness) * scatteringDiffuse;
					...
					fixed3 color = (diffuseTerm * _LightColor0 * diffuseShadowTerm + UNITY_LIGHTMODEL_AMBIENT) * albedo +
